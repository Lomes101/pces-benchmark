...
def taipan_eval(task: str, **kwargs) -> float:
    ...
    """
    if task == 'a':
        # Autocorrelacion de grados — Taipan v4
        adj = kwargs['adj'].astype(float)
        deg = adj.sum(axis=0)
        shift = np.roll(deg, 1)
        corr = np.corrcoef(deg, shift)[0, 1] if deg.std() > 1e-8 else 0.0
        # Wasserstein espectral Laplaciano — Taipan v6
        adj_f = kwargs['adj'].astype(float)
        deg_vec = adj_f.sum(axis=1)
        L = np.diag(deg_vec) - adj_f  # Laplaciano
        eigvals = np.sort(np.linalg.eigvalsh(L))
        # Comparar histogramas de autovalores: mitad inferior vs superior
        mid = len(eigvals) // 2
        p = eigvals[:mid] + 1e-8
        q = eigvals[mid:] + 1e-8
        p = p / p.sum()
        q = q / q.sum()
        # Wasserstein 1D = integral |CDF_p - CDF_q|
        cdf_p = np.cumsum(p)
        cdf_q = np.cumsum(np.interp(
            np.linspace(0, 1, len(cdf_p)),
            np.linspace(0, 1, len(q)),
            np.cumsum(q)
        ))
        wasserstein = float(np.mean(np.abs(cdf_p - cdf_q)))
        pred = int(wasserstein > 0.1)
        true_switch = kwargs.get('true_switch', 0)
        return float(pred == (true_switch > 0))
